...
use crate::inc::{ChunkReaderTrait, ChunkWriterTrait, LoadFromPathTrait};

use super::{MemChunk, mem_chunk::MemChunkBuilder};
...
impl ChunkReaderTrait for Data {
    ...
    async fn get(&self, _: &ChunkId, offset: usize, content: &mut [u8]) -> NearResult<usize> {
        match self.0.data
                  .as_ref() {
            Some(data) => {
                data.read_to_end(offset, content.len())
                    .map(| data | {
...
impl Manager {
    ...
    //     if newly {
    //         let text = 
    //             match MemChunkBuilder::new(chunk)
    //                     .load_from_path(self.0.root_path.as_path())
    //                     .await {
...
//                   .get_mut(chunk) {
//             Some(data) => {
//                 data.as_mut(chunk).write_all(offset, content)
//             }
