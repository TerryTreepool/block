...
int set_sock_recv_size(int dd, int optval) {
    // set the receive buffer size
    socklen_t optlen = sizeof(optval);
    int status = setsockopt(dd, SOL_SOCKET, SO_RCVBUF, &optval, optlen);
    if (status < 0) {
        // handle error
        return -1;
    }
...
int hci_le_set_advertising_paramters(int dd, uint16_t min_interval, uint16_t max_interval, uint16_t to) {
...
int hci_le_advertising_data(int dd, const uint8_t* data, uint8_t size, uint16_t to) {
	...
	adv_data_cp.length = size;
	memcpy(adv_data_cp.data, data, size);
	
	rq.ogf = OGF_LE_CTL;
	rq.ocf = OCF_LE_SET_ADVERTISING_DATA;
	rq.cparam = &adv_data_cp;
	rq.clen = adv_data_cp.length+1;
	rq.rparam = &status;
	rq.rlen = 1;
...
int hci_le_enable_advertising(int dd, uint8_t enable, uint16_t to) {
					...
					uint8_t own_type, uint8_t filter, int to)
{
	...
	param_cp.type = type;
	param_cp.interval = htobs(interval);
	param_cp.window = htobs(window);
