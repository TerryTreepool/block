...
/// named
/// object desc content
pub trait NamedObjectDescTrait: ObjectDescTrait + Serialize + Deserialize {
    fn create_timestamp(&self) -> u64;
    fn expired_time(&self) -> Option<u64>;

    fn owner(&self) -> Option<Self::OwnerObj>;
    fn area(&self) -> Option<Self::AreaObj>;
    fn author(&self) -> Option<Self::AuthorObj>;
    fn public_key(&self) -> Option<Self::PublicKeyObj>;

    fn desc<'a>(&'a self) -> &'a dyn ObjectDescTrait<OwnerObj = Self::OwnerObj, AreaObj = Self::AreaObj, AuthorObj = Self::AuthorObj, PublicKeyObj = Self::PublicKeyObj>;
}
...
pub trait NamedObjectBodyTrait: ObjectBodyTrait + Serialize + Deserialize {
    fn body<'a>(&'a self) -> &'a dyn ObjectBodyTrait;
}

pub trait NamedObjectTrait: NamedObjectDescTrait + NamedObjectBodyTrait {
    fn named_object_desc<'a>(&'a self) -> &'a dyn NamedObjectDescTrait<OwnerObj = Self::OwnerObj, AreaObj = Self::AreaObj, AuthorObj = Self::AuthorObj, PublicKeyObj = Self::PublicKeyObj>;
...
// impl<DESC, BODY> ObjectTrait for NamedObject<DESC, BODY>
// where DESC: ObjectDescTrait + Serialize + Deserialize,
//       BODY: ObjectBodyTrait + Serialize + Deserialize {
//     fn id(&self) -> &ObjectId {
//         &self.object_id        
