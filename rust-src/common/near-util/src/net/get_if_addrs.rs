...
mod getifaddrs_posix {
    ...
    pub fn get_if_addrs() -> io::Result<Vec<Interface>> {
        ...
        unsafe {
            #[allow(invalid_value)]
            let mut ifaddrs = mem::MaybeUninit::uninit().assume_init();
...
pub fn get_if_sockaddrs() -> NearResult<Vec<IpAddr>> {
    ...
    let interfaces = interfaces.map_err(| _e | {
        let error_string = format!("failed get address with err = {}", stringify!(_e));
        error!("error_string");
    ...
    for i in interfaces {
        let adr = match i.addr {
            IfAddr::V4(ipv4) => IpAddr::from(ipv4.ip),
            IfAddr::V6(ipv6) => IpAddr::from(ipv6.ip),
        };
        addrs.push(adr);
    }
...
#[test]
fn test() {
    let interfaces = get_if_sockaddrs().unwrap();
    for interface in interfaces {
        let addr_str = match interface {
...
fn test_domain() {
    let h = 
        unsafe {
            let mut _data = std::mem::MaybeUninit::uninit().assume_init();
            if WSAStartup(514, &mut _data) != 0 {
                unreachable!()
            }
            let name = CString::new("www.baidu.com").unwrap().into_raw();
            gethostbyname(name)
        };

    let ptr_h = unsafe {&*h};

    let name = unsafe { CString::from_raw(ptr_h.h_name) };
    let addr = unsafe { 
        // let mut inet = std::mem::MaybeUninit::uninit().assume_init();
        let mut inet: ::std::mem::MaybeUninit<in_addr> = ::std::mem::MaybeUninit::uninit();
        // for addr in
        //     c_linked_list::CLinkedListMut::from_ptr(ptr_h.h_addr_list, | a | {
        //         a    
        //     }).iter() {
            
        // }
        // let y  =(*ptr_h.h_addr_list);

        // let x = inet.as_mut_ptr();

        // let x = std::ptr::read(ptr_h.h_addr_list);
        // // std::ptr::copy(src, dst, count)

        // inet.assume_init_read()
        // std::ptr::copy((*ptr_h.h_addr_list), inet.as_mut_ptr().write_bytes(val, count), std::mem::size_of::<in_addr>());
        // inet_ntoa(*ptr_h.h_addr_list)
        // CString::from_raw(*ptr_h.h_addr_list) 
    };
    // for addr in unsafe { c_linked_list::CLinkedListMut::from_ptr(ptr_h.h_addr_list, | a | a)}.iter() {
    ...
    unsafe {
        let sun_b = addr.S_un.S_un_b();
        println!("{}.{}.{}.{}", sun_b.s_b1, sun_b.s_b2, sun_b.s_b3, sun_b.s_b4);
    }
}
