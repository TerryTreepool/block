...
use std::{sync::Arc, collections::BTreeMap, path::{PathBuf}, io::{SeekFrom, ErrorKind}};

use async_std::{io::{WriteExt, prelude::SeekExt, BufWriter}, };
use dataagent_util::Helper;
use futures::AsyncReadExt;
use log::{debug, error};
use nds::NdsStack;
use near_base::{NearResult, NearError, ErrorCode, };
use near_core::{get_temp_path};
use near_transport::Stack;
use near_util::{BufReadWithLimit};
use serde_json::Number;
use tide::convert::Deserialize;

use super::{Config, 
            nds_process::NdsManager, 
            p::{FileArticleManager, FileArticlePtr, HttpResult, ToHttpResult}, 
            http_server::{HttpServer, HttpEventTrait, UrlRequest}
    };
...
const DEFAULT_SERVICE_NAME: &'static str = "gateway";
...
impl App {
    pub fn new(stack: Stack, config: Config) -> NearResult<Self> {

        let nds_stack = NdsStack::open(DEFAULT_SERVICE_NAME.to_owned(), stack, Default::default())?;

        let sqlx_helper = Arc::new(Helper::new(config.db.as_path(), Default::default())?);

        let app = Self(Arc::new(AppImpl {
            config,
            nds_manager: NdsManager::new(nds_stack),
            sqlx_helper: sqlx_helper.clone(),
        }));

        let app_clone = app.clone();
        async_std::task::spawn(async move {
            let _ = App::init_data(sqlx_helper)
                        .await
                        .map_err(| err | {
                            panic!("{}", err);
                        });
    ...
    pub fn nds_stack(&self) -> NdsStack {
        self.0.nds_manager.nds_stack().clone()
    }

    async fn init_data(sqlx: Arc<Helper>) -> NearResult<()> {
        let file_article =
            String::from(r#"CREATE TABLE IF NOT EXISTS file_article (
                ...
                "create_time" TIME,
                "flag" INTEGER);"#);
        sqlx.add_sql("create_file_article", file_article)?;
        sqlx.execute("create_file_article", Default::default()).await?;
        ...
        sqlx.add_sql("insert_file_article", insert_file_article)?;
        ...
        sqlx.add_sql("query_file_article", query_file_article)?;

        sqlx.query_all("query_file_article", BTreeMap::default())
                .await
                .iter()
                .for_each(| v | {
                    let _ = v.iter()
                            .for_each(| map | {
                                if let Some(file_id) = map.get("file_id") {
                                    ...
                                    FileArticleManager::get_instance().add_file(FileArticlePtr::with_info(file_id.clone(), uid, file_name, file_size, flag));
                                }
                            });
                });
...
impl App {
    ...
    async fn create_file_article(&self, req: UrlRequest<'_>) -> NearResult<Box<dyn ToHttpResult>> {
        #[derive(Deserialize)]
        struct FileArticleReq {
            file_name: String,
            file_size: u64
        }

        let file_article = req.body.into_form::<FileArticleReq>().await.map_err(|e| {
            NearError::new(ErrorCode::NEAR_ERROR_INVALIDPARAM, format!("failed to convert form data with err: {}", e))
        })?;

        let file_article =
            FileArticlePtr::new(req.uid.to_string(),
                                file_article.file_name,
                                file_article.file_size
            )?;
        // let file_article =
        //     FileArticlePtr::new(req.uid.to_string(),
        //                         file_name.to_string(),
        ...
        let _ = FileArticleManager::get_instance().add_file(file_article.clone());
    ...
    async fn upload_file(&self, req: UrlRequest<'_>) -> NearResult<Box<dyn ToHttpResult>> {
        ...
        let file_article = 
        FileArticleManager::get_instance()
            .get_file(file_id)
        ...
        const FIX_BUFFER_MAX_LEN: usize = 1024 * 1024;

        let file_length = file.seek(SeekFrom::End(0)).await?;
        let mut buffer = Vec::with_capacity(FIX_BUFFER_MAX_LEN);
        let mut writed_length = 0usize;

        let mut limit_reader = BufReadWithLimit::new(FIX_BUFFER_MAX_LEN as u64, req.body.into_reader());

        let _ = loop {
            match limit_reader.read_to_end(&mut buffer).await {
                Ok(bytes) => {
                    if bytes > 0 {
                        ...
                        file.write(&buffer[..bytes])
                            .await
                            .map_err(| e | {
                                let error_string = format!("failed write [{}] with err {}", file_id, e);
                                error!("{}", e);
                                NearError::new(ErrorCode::NEAR_ERROR_SYSTERM, error_string)
                            })?;

                        if writed_length % FIX_BUFFER_MAX_LEN == 0 {
                            limit_reader.reset();
                        }
                    } else {
                        break(Ok(writed_length))
                    }
                }
                Err(e) => {
                    match e.kind() {
                        ErrorKind::UnexpectedEof => { break(Ok(writed_length)) }
                        _ => {
                            let error_string = format!("failed read_exact [{}] with err {}", file_id, e);
                            error!("{}", error_string);
                            break(Err(NearError::new(ErrorCode::NEAR_ERROR_SYSTERM, error_string)));
                        }
                    }
                }
            }
        ...
        if file_length + writed_length as u64 == file_article.file_size() {
            let _ = file_article.finished();
            // finished
    ...
    async fn get_file_list(&self, req: UrlRequest<'_>) -> NearResult<Box<dyn ToHttpResult>> {
        ...
        let (file_list, page_count) = FileArticleManager::get_instance().filter_file(index);
    ...
    async fn get_file_content(&self, req: UrlRequest<'_>) -> NearResult<Box<dyn ToHttpResult>> {
        ...
        let _ = FileArticleManager::get_instance()
                    .get_file(file_id)
                    .ok_or_else(|| NearError::new(ErrorCode::NEAR_ERROR_NOTFOUND, format!("[{}] not found it.", file_id) ))?;
