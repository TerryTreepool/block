...
impl RuntimeConfig {
    ...
    pub fn set_core_desc(&self, core_name: &str) {
        self.0.write().unwrap().core_desc = get_data_path().join(format!("{}.{DESC_SUFFIX_NAME}", core_name, ));
    }
    ...
    pub fn set_device_desc(&self, device_name: &str) {
        ...
        mut_self.device_desc = get_data_path().join(format!("{}.{DESC_SUFFIX_NAME}", device_name));
    }
