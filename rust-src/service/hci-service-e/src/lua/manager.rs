...
use std::{sync::Arc, path::PathBuf, collections::BTreeMap, ops::Add,};
...
use super::{data::Data, built_in::built_inner::BuiltInner, utils};
...
impl InnerLua {
    ...
    pub async fn call(&self, function: &str, params: Data) -> NearResult<Vec<u8>> {
        self.lua
            ...
            .context(| ctx | {
                ...
                debug!("debug_string:{:?}", tb.len());
                for (k, v) in params.clone_map() {
                    debug_string = debug_string.add(format!("thing_data[{k}]={v},").as_str());
                    let _ = tb.set(k, v);
                }
                debug!("debug_string:{debug_string}");
    ...
    pub async fn parse(&self, function: &str, data: Vec<u8>, input_data: Data, output_data: Data) -> NearResult<bool> {
        self.lua
            .lock().await
            .context(| ctx | {
                let globals = ctx.globals();
                ...
                // input userdata
                {
                    let userdata = ctx.create_userdata(input_data).unwrap();
                    globals.set("input_data", userdata)
                        .map_err(| e | {
                            let error_string = format!("file set input_data with err: {e}");
                            error!("{error_string}");
                            NearError::new(ErrorCode::NEAR_ERROR_3RD, error_string)
                        })?;
                ...
                // output userdata
                {
                    let userdata = ctx.create_userdata(output_data).unwrap();
                    globals.set("output_data", userdata)
                        .map_err(| e | {
                            let error_string = format!("file set output_data with err: {e}");
                            error!("{error_string}");
                            NearError::new(ErrorCode::NEAR_ERROR_3RD, error_string)
                        })?;    
                }
...
impl Manager {
    async fn parse_data(&self, module: &str, input: Vec<u8>, input_data: Data) -> NearResult<Data> {
        let module = {
            self.0.modules.get(module)
                .ok_or_else(|| {
                    let error_string = format!("Not found {module} module");
        ...
        let r = module.parse(TaskModule::AnalizeData.to_str(), input, input_data, output.clone()).await?;

        if r { Ok(output) } else { Err(NearError::new(ErrorCode::NEAR_ERROR_3RD, "error")) }
    }

    pub async fn analyze_data(&self, input: Vec<u8>, input_data: Data) -> NearResult<Data> {
        let built_inner = 
            self.0.built_inner
                .as_ref()
                .ok_or_else(|| {
        ...
        self.parse_data(module.as_str(), input, input_data).await
    }
